...
public class OrderGeneratorService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            var nextOrderDelay = TimeSpan.FromSeconds(_random.Next(3, 6));
            ...
            {
                shouldAddOrder = _orders.Count(o => o.Status is not OrderStatus.Delivered) < 10; // Prevent too many orders accumulating (max 10 at once)
                if (shouldAddOrder && _orders.Count >= 10 && _orders.FirstOrDefault()?.Status is OrderStatus.Delivered)
                {
                    _orders.RemoveAt(0);
                }
            }
